...
class SeemZ:
	def __init__(self):
		fileTheme=open("theme.list","r")
		interface = gtk.Builder()
		interface.add_from_file(home+'/SeemZ/seemz.glade')
		...
		fileTheme.close()
		fileThemeActive=open("theme.active","r")
	...
	def theme_choosen(self,widget):
		...
		oldFile=open("theme.active","r")
		oldTheme=oldFile.read()
		oldFile.close()
		mvtheme=open("theme.list", "r")
		writeList=""
		for old in mvtheme:
			if themeChoose!=old :
				writeList+=old
		...
		print(writeList)
		mvtheme.close()
		mvtheme=open("theme.list", "w+")
		mvtheme.write(writeList)
		mvtheme.close()
		oldFile=open("theme.active","w+")
		oldFile.write(themeChoose)
		oldFile.close()

		#code clean :)))))#########
		
		file=open("theme.active","w+")
